...
# 5. Perform linear regression using the lm() function. In the lm() function, pass in all six
...
MechaCarCoil <- read.csv(file = "Suspension_Coil.csv", check.names = F, stringsAsFactors = F)

# 3. Create a total_summary dataframe using the summarize() function to get the 
# mean, median, variance, and standard deviation of the suspension coil’s PSI column.
total_summary <- MechaCarCoil %>% summarize(Mean=mean(PSI),
                                            Median=median(PSI),
                                            Variance=var(PSI),
                                            StdDev=sd(PSI))
...
lot_summary <- MechaCarCoil %>% group_by(Manufacturing_Lot) %>% summarize(Mean=mean(PSI),
                                                                          Median=median(PSI),
                                                                          Variance=var(PSI),
                                                                          StdDev=sd(PSI),
                                                                          .groups='keep')
...
t.test(MechaCarCoil$PSI,mu=1500)


# 2. Write 3 more RScripts using the t.test() function and its subset() argument to determine if 
# the PSI for each manufacturing lot is statistically different from the population mean of 1,500 pounds per square inch.
t.test(subset(MechaCarCoil,Manufacturing_Lot=="Lot1")$PSI,mu = 1500)
t.test(subset(MechaCarCoil,Manufacturing_Lot=="Lot2")$PSI,mu = 1500)    
t.test(subset(MechaCarCoil,Manufacturing_Lot=="Lot3")$PSI,mu = 1500)
